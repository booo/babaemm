...
static OSMPBF__BlobHeader *read_header(FILE *input, void *buf)
{
  OSMPBF__BlobHeader *header_msg;
  ...
  header_msg = osmpbf__blob_header__unpack (NULL, length, buf);
  if (header_msg == NULL) {
...
static OSMPBF__Blob *read_blob(FILE *input, void *buf, int32_t length)
{
  OSMPBF__Blob *blob_msg;
  ...
  blob_msg = osmpbf__blob__unpack (NULL, length, buf);
  if (blob_msg == NULL) {
...
static size_t uncompress_blob(OSMPBF__Blob *bmsg, void *buf, int32_t max_size)
{
...
{
  OSMPBF__HeaderBlock *hmsg = osmpbf__header_block__unpack (NULL, length, data);
  if (hmsg == NULL) {
    fprintf(stderr, "Error unpacking HeaderBlock message\n");
    return 0;
  }

  osmpbf__header_block__free_unpacked (hmsg, &protobuf_c_system_allocator);

  return 1;
}

int processOsmDataNodes(OSMPBF__PrimitiveGroup *group, OSMPBF__StringTable * string_table, double lat_offset, double lon_offset, double granularity)
{
  unsigned node_id;
  for (node_id = 0; node_id < group->n_nodes; node_id++) {
    OSMPBF__Node *node = group->nodes[node_id];
    double lat, lon;
...
int processOsmDataDenseNodes(OSMPBF__PrimitiveGroup *group, OSMPBF__StringTable *string_table, double lat_offset, double lon_offset, double granularity)
{
  if (group->dense) {
    unsigned l = 0;
    long int deltaid = 0;
    ...
    OSMPBF__DenseInfo *denseinfo = dense->denseinfo;
    ...
    while (dense->keys_vals[l] != 0 && l < dense->n_keys_vals) {
        //printf("k: %s, v: %s\n", string_table->s[dense->keys_vals[l]].data, string_table->s[dense->keys_vals[l+1]].data);
        /*addProtobufItem(&(osmdata->tags),
    ...
    l += 1;
      ...
      struct tm tim;
      time_t tstamp;
      tstamp = time(deltatimestamp);*/
      const char* tags = "";
      fprintf(stdout,"%ld;%d;%ld;2010-09-21 19:34:08;%lu;%s;SRID=4326\\;POINT(%f %f)\n",deltaid,denseinfo->version[node_id],deltauid,/*deltatimestamp,*/deltachangeset,tags,lat,lon);
      /*if(deltauid != -1) {
            printf("%s", string_table->s[deltauser_sid].data);
      }*/
...
int processOsmDataWays(OSMPBF__PrimitiveGroup *group, OSMPBF__StringTable *string_table)
{
    unsigned way_id;
    char nodes[2048*32];
    char tmp[32];
  ...
  for (way_id = 0; way_id < group->n_ways; way_id++) {
    OSMPBF__Way *way = group->ways[way_id];
    long int deltaref = 0;
    OSMPBF__Info *info = way->info;
    //resetList(&(osmdata->tags));
    ...
    */
    ...
    if(ref_id < way->n_refs) {
        sprintf(tmp,"%lld}",way->refs[ref_id+1]);
    } else {
        sprintf(tmp,"}");
    }
    strcat(nodes,tmp);
    ...
    const char* tags = "";
    fprintf(stdout,"%lld;%d;%d;2010-09-21 19:34:08;%lld;%s;%s\n",way->id,info->version,info->uid,/*deltatimestamp,*/info->changeset,tags,nodes);
    //free(nodes);
    //free(tmp);
...
*/
int processOsmData(void *data, size_t length)
{
  OSMPBF__PrimitiveBlock *pmsg = osmpbf__primitive_block__unpack (NULL, length, data);
  if (pmsg == NULL) {
  ...
  for (j = 0; j < pmsg->n_primitivegroup; j++) {
    OSMPBF__PrimitiveGroup *group = pmsg->primitivegroup[j];
    OSMPBF__StringTable *string_table = pmsg->stringtable;

    //if (!processOsmDataNodes(group, string_table, lat_offset, lon_offset, granularity)) return 0;
    //if (!processOsmDataDenseNodes( group, string_table, lat_offset, lon_offset, granularity)) return 0;
    if (!processOsmDataWays(group, string_table)) return 0;
    /*if (!processOsmDataRelations(group, string_table)) return 0; */
  }

  osmpbf__primitive_block__free_unpacked (pmsg, &protobuf_c_system_allocator);
...
int streamFilePbf(char *filename)
{
  void *header = NULL;
  void *blob = NULL;
  char *data = NULL;
  FILE *input = NULL;
  OSMPBF__BlobHeader *header_msg = NULL;
  OSMPBF__Blob *blob_msg = NULL;
  size_t length;
  ...
  do {
    ...
    goto err;
      ...
      if (!processOsmData(data, length)) {
    goto err;
      }
    }

    osmpbf__blob__free_unpacked (blob_msg, &protobuf_c_system_allocator);
    osmpbf__blob_header__free_unpacked (header_msg, &protobuf_c_system_allocator);
  } while (!feof(input));
...
int main(int argc, char ** argv) {
    ...
    if(argc == 2) {
        streamFilePbf(argv[1]);
    } else {
        streamFilePbf("/dev/stdin");
    }
    finishGEOS();
}
